...
export default function StickyMobileBar() {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-x-0 bottom-0 z-40 flex gap-3 px-4 sm:hidden"
      style={{ paddingBottom: "calc(1rem + env(safe-area-inset-bottom))" }}
    >
      <a
        href={`tel:${BRAND.phoneIntl.replace(/\s/g, "")}`}
        onClick={() => trackEvent("call_click", { source: "sticky_mobile_bar" })}
        className="flex flex-1 items-center justify-center gap-2 rounded-full bg-red py-3 text-[0.92rem] font-semibold text-paper shadow-[0_10px_28px_-6px_rgba(184,62,61,0.6)] transition-transform active:scale-[0.97]"
      >
      ...
      <a
        ...
        className="flex flex-1 items-center justify-center gap-2 rounded-full py-3 text-[0.92rem] font-semibold text-paper shadow-[0_10px_28px_-6px_rgba(37,211,102,0.6)] transition-transform active:scale-[0.97]"
